...
def stitch_pattern_pdf(
    ...
    print(f"Loading pages {first_page}–{last_page} from {pdf_path} at {dpi} DPI...")
    ...
    if last_page > doc.page_count:
        ...
        print(f"Adjusting last_page to {last_page} (document has {doc.page_count} pages)")
    
    pages = []
    for page_num in range(first_page - 1, last_page):  # fitz uses 0-based indexing
    ...
    for i, tile in enumerate(cropped_pages):
        ...
        existing_region = big_img.crop((x, y, x + tile_w, y + tile_h)).convert("RGB")
        ...
        blended_array[overlap_mask] = (existing_array[overlap_mask] * 0.5 + 
                                     tile_array[overlap_mask] * 0.5).astype(np.uint8)
...
def main():
    ...
    parser.add_argument("output_path", help="Path to output image (e.g. stitched.png)")
    ...
    parser.add_argument("--margin-vertical-in", type=float, default=None, 
                       help="Vertical margin (top and bottom) in inches")
    parser.add_argument("--margin-horizontal-in", type=float, default=None,
